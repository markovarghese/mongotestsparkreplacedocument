...
docker-compose exec mongos1 bash -c "echo -e 'use testDb\ndb.createCollection(\"SparkNonShardedCollection\")' | mongo"
echo "create a sharded collection SparkShardedCollection on our sharded database"
docker-compose exec mongos1 bash -c "echo -e 'use testDb\ndb.createCollection(\"SparkShardedCollection\")' | mongo"
echo "shard our collection on a field named shardingField"
docker-compose exec mongos1 bash -c "echo 'sh.shardCollection(\"testDb.SparkShardedCollection\", {\"shardingField\" : \"hashed\"})' | mongo"
