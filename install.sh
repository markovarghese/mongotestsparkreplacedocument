...
echo "Run the Spark application using the dockerised spark server to import ${jsonfile} into NonShardedCollection"
docker run -v $(pwd)/sampledata:/sampledata  -v $(pwd)/target:/target -it --rm --network mongo_network  spark3.0.1-scala2.12-hadoop3.2.1:latest spark-submit --deploy-mode client --class org.example.App /target/simplespark-1.0-SNAPSHOT-jar-with-dependencies.jar ${jsonfile} "NonShardedCollection"
echo "Run the Spark application using the dockerised spark server to import ${jsonfile} into ShardedCollection"
docker run -v $(pwd)/sampledata:/sampledata  -v $(pwd)/target:/target -it --rm --network mongo_network  spark3.0.1-scala2.12-hadoop3.2.1:latest spark-submit --deploy-mode client --class org.example.App /target/simplespark-1.0-SNAPSHOT-jar-with-dependencies.jar ${jsonfile} "ShardedCollection"